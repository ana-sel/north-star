-- Compass v1 · local SQLite schema
-- Single-user device DB. All timestamps are ISO 8601 UTC strings.
-- All ids are UUIDs (TEXT). Enums are TEXT with CHECK constraints.
-- Every table mirrors the future Postgres shape minus `user_id` — a v2
-- sync script adds that column and .upserts to Supabase.
...
-- Independent from sleep — user can drop check-ins during the day.
...
-- ─── Habits ───────────────────────────────────────────────────────
-- User-defined habits. Each habit belongs to a ritual (morning/evening/none).
CREATE TABLE IF NOT EXISTS habits (
  ...
  days_mask    INTEGER,                        -- bitmask Mon=1…Sun=64, when cadence='custom'
...
-- One completion row per (habit, local_date). Idempotent by UNIQUE.
CREATE TABLE IF NOT EXISTS habit_completions (
  id            TEXT PRIMARY KEY,
  habit_id      TEXT NOT NULL REFERENCES habits(id) ON DELETE CASCADE,
  local_date    TEXT NOT NULL,                 -- YYYY-MM-DD
  completed_at  TEXT NOT NULL,                 -- ISO UTC
  timezone      TEXT NOT NULL,
...
-- ─── Plan · goals + tasks ─────────────────────────────────────────
-- Goals form a tree (self-referencing parent_id). Root nodes have parent_id NULL.
CREATE TABLE IF NOT EXISTS goals (
  id          TEXT PRIMARY KEY,
  parent_id   TEXT REFERENCES goals(id) ON DELETE CASCADE,
  title       TEXT NOT NULL,
  description TEXT,
  pillar      TEXT,                            -- inner | body | money | craft | together | frontier | admin
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  done_when    TEXT,                           -- how you'll know it's done
...
-- ─── Paths ────────────────────────────────────────────────────────
-- The 41 qualities themselves are bundled read-only content (see
-- paths-seed.ts) — no `paths` table. Only user *engagement* is stored.
CREATE TABLE IF NOT EXISTS path_progress (
  path_id       TEXT PRIMARY KEY,               -- matches DATA[id] in bundled seed
  ...
  claimed_at    TEXT,                            -- user-claimed as integrated
  notes         TEXT,
  updated_at    TEXT NOT NULL
);

-- Every practice tick — spaced returns trail comes from here.
CREATE TABLE IF NOT EXISTS path_returns (
  id          TEXT PRIMARY KEY,
  path_id     TEXT NOT NULL,
  local_date  TEXT NOT NULL,                    -- YYYY-MM-DD
  logged_at   TEXT NOT NULL,                    -- ISO UTC
  timezone    TEXT NOT NULL,
  camp_at_return INTEGER NOT NULL CHECK (camp_at_return BETWEEN 1 AND 3),
  note        TEXT,
  UNIQUE (path_id, local_date)                  -- one return per day per path
);
CREATE INDEX IF NOT EXISTS idx_path_returns_path ON path_returns(path_id, local_date DESC);

-- ─── User preferences ─────────────────────────────────────────────
-- Single-row key/value store. Simple, extensible.
...
-- ─── Journal notes (optional free text) ───────────────────────────
