...
import { localDateForInstant } from '../lib/time';
...
export async function getTodaySleep(localDate: string): Promise<SleepEntry | null> {
  const rows = await query<Record<string, unknown>>(
    'SELECT * FROM sleep_entries ORDER BY sleep_end_utc DESC',
  );
  const row = rows.find((candidate) =>
    localDateForInstant(new Date(candidate.sleep_end_utc as string), candidate.timezone as string) === localDate,
  );
